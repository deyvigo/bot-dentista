...
import { intervalCancelDateInSchedule, validateDoctorResponses } from '../../services/validateResponses'
...
export const cancelDate = async (sock: WASocket, messageInfo: proto.IWebMessageInfo, session: Session) => {
  ...
  switch (session.step) {
    case 0:
      ...
      break
    case 1:
      if (await validateDoctorResponses(sock, session, messageInfo, `, siendo hoy ${formatDate(new Date())}; no tiene relación con días y fechas`)) return
    ...
    case 3:
      ...
      session.payload = doctorPayload
      // TODO: REFACTOR OF PROMPT => Generate a json { day, init, end } and pass to intervalCancelDateInSchedule() to generate intervals to cancel day
      // TODO: validar que el día y la hora estén dentro del horario del doctor
      if (!await intervalCancelDateInSchedule(doctorPayload, sock, from, session)) return
