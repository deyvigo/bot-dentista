...
export class DateRepository {
  ...
  static getClientDates = async (number_client: string) => {
    try {
      const query = `
      SELECT d.id_date, d.day, d.hour, CONCAT(dc.name, ', ', dc.last_name) as doctor
      FROM date d
      JOIN client c ON d.id_client = c.id_client
      JOIN doctor dc ON d.id_doctor = dc.id_doctor
      WHERE c.phone = ? AND d.state = 'libre';
      `
  ...
  static deleteDateClient = async (id_date: string) => {
    try {
      const query = 'DELETE FROM date WHERE id_date = ?;'
      const [result] = await dbConnection.query<ResultSetHeader>(query, [id_date])
